...
interface PaceZoneDurationBarChartProps {
  data: PaceZoneStat[];
}

/**
 * 基于 laps 统计的各配速区间跑步时间柱状图。
 * Y 轴为配速区间（Z1-Z5），X 轴为时间（分钟）。
 */
export default function PaceZoneDurationBarChart({ data }: PaceZoneDurationBarChartProps) {
  ...
  useEffect(() => {
    ...
    const zoneLabels = zones.map((z) => PACE_ZONE_NAMES[z]);
      ...
      series: [
        {
          ...
          data: zoneLabels.map((_, i) => ({
            ...
            itemStyle: {
              color: PACE_ZONE_COLORS[zones[i]],
            },
          })),
          barMaxWidth: 36,
        },
